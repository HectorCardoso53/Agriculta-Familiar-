...
function abrirModalForn(pid) {
  state.projetoSelecionado = pid;
  ['f-nome','f-cpf','f-dap'].forEach(id => { document.getElementById(id).value = ''; });
...
async function excluirFornecedor(fid) {
  if (!confirm('Excluir fornecedor e todos os seus produtos?')) return;
  try {
    const prods = await load('produtos');
    for (const p of prods.filter(x => x.fornecedorId === fid)) {
      await deleteDoc('produtos', p.id);
    }
    await deleteDoc('fornecedores', fid);
